...
IPADDRESS=$(aws ec2 describe-instances --instance-ids ${INSTANCE_ID} --output table | grep PrivateIpAddress | head -n 1 | awk '{print $4}')
 ...
 sed -e "s/COMPONENT/${NAME}/" -e "s/IPADDRESS/${IPADDRESS}/" record.json
aws route53 change-resource-record-sets --hosted-zone-id Z02287642UQYKFGW743KN --change-batch file:// record.json
